...
describe("TFLLint", () => {
  ...
  describe("Node Validation", () => {
    ...
    it("should report error for missing node id", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(issues.some((issue) => issue.code === "missing-node-id")).toBe(
        true
      );
    });
    ...
    it("should report error for duplicate node ids", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(issues.some((issue) => issue.code === "duplicate-node-id")).toBe(
        true
      );
    });
    ...
    it("should report error for invalid node type", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(issues.some((issue) => issue.code === "invalid-node-type")).toBe(
        true
      );
    });
    ...
    it("should report error for missing required inputs", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "missing-required-input")
      ).toBe(true);
    });
    ...
    it("should warn about unknown inputs", () => {
      ...
      expect(
        issues.some(
          (issue) =>
            issue.code === "unknown-input" && issue.severity === "warning"
        )
      ).toBe(true);
    });
    ...
    it("should report error for invalid output structure", () => {
        ...
        nodes: [
          {
            ...
            outputs: [{ id: "latest tweets", isDeleted: "invalid" as any }],
          },
        ],
        edges: [],
      };
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "invalid-output-isdeleted")
      ).toBe(true);
    });
    ...
    it("should warn about position overlap", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "node-position-overlap")
      ).toBe(true);
    });
  });
  ...
  describe("Edge Validation", () => {
    ...
    it("should report error for missing edge fields", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(issues.some((issue) => issue.code === "missing-edge-source")).toBe(
        true
      );
    });
    ...
    it("should report error for non-existent source node", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "invalid-edge-source-node")
      ).toBe(true);
    });
    ...
    it("should report error for invalid source handle", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "invalid-edge-source-handle")
      ).toBe(true);
    });
    ...
    it("should report error for invalid target handle", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(
        issues.some((issue) => issue.code === "invalid-edge-target-handle")
      ).toBe(true);
    });
  });
  ...
  describe("Flow Integrity", () => {
    ...
    it("should detect circular dependencies", () => {
      ...
      const issues = lint.lintFlow(data);
      expect(issues.some((issue) => issue.code === "circular-dependency")).toBe(
        true
      );
    });
  });
